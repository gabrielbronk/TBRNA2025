...
## WARNING: THIS SCRIPT ASSUMES THERE IS ONLY ONE COVARIATE IN CovariateNames. My custom fitting
## script is not written for more than one covariate.


##------------------------------------------------------------------------------
# DEGNameToRun = c('ENSG00000152969')
# 
# DEGListOfNames = rownames(FinalDEGDF)
# LoopNumbers = which(DEGListOfNames == DEGNameToRun)
# TaskIDNumber = LoopNumbers
...
## Use Option 2 to run just the RNA you input above.
...
## WARNING: You may need to adjust the range and intervals of I and B 
...
## The bottom of this script will spit out information that tells you whether
## the range and intervals need adjustment:
                                                    # For running short tests of code:  
#IArray = seq(-7,6,0.04) #seq(-3,3,0.04)                 # seq(-3,3,0.1)                     
#BArray = seq(-0.3,0.9,0.002)   #seq(-1,1,0.01)           # seq(-1,1,0.02)   
#SArray = seq(-2,2,0.02)  #seq(-5,5,0.05)              # seq(-5,5,0.25)     

NumberOfIsForZero = 5000#10000       # I had used 2000 but the MAP estimate was imprecise #300#1500#300
NumberOfBsForZero = 15000#30000       # I had used 6000 but the MAP estimate was imprecise #2000#1500#300

NumberOfIsForOne = 250#250      #400 #2*100        ## for 24 hours or so, this was 300
NumberOfBsForOne = 1000      #2*300       ## this was 1000
NumberOfSs = 300#400             #2*100              ## this was 400

NumOfBins = 300

## This loads up the CorrectedCountsMatrix, which is the counts matrix for all blood samples. 
## The counts matrix was corrected for technical variables and batch (see Step2.R):
...
##------------------------------------------------------------------------------
    ## WARNING: THIS SECTION ASSUMES THERE IS ONLY ONE COVARIATE IN CovariateNames. My custom fitting
    ## script will not work if there is more than one covariate.
    ## If you have more than one covariate in CovariateNames, then rewrite this section to accomodate
    ## finding the NA values in multiple covariates (this would be for the BRGLM package fitting, not my custom
    ## fitting because my custom
    ## fitting would take too long to run with more than one covariate).
...
##==============================================================================  
## Using packages to perform logistic regression:
##==============================================================================
...
if (NumberOfCovariatesForRNA == 0) {  
  ...
  ## for relative risk for an expression of xmax vs an expression of xmin (normalized, 
  ...
  ##============================================================================
  ## This section finds the 95% CIs for RRMax correctly by integrating over all of 
  ...
  if (NumberOfCovariatesForRNA == 1) {  
    ...
    #Zeros = rep(0, NumberOfIs*NumberOfBs*NumberOfSs)
    ## The tensors will be initialized by filling them entirely with 0.
    #LogPosteriorMatrix = array(Zeros,
                               #c(NumberOfIs,NumberOfBs,NumberOfSs))
    
    
    ########################### Do I need this?????:
    #PriorMatrix = array(Zeros,
                       # c(NumberOfIs,NumberOfBs,NumberOfSs))
    # This is for investigating the -Infs and NAs
    
    #LogLikelihoodMatrix = array(Zeros,
                               # c(NumberOfIs,NumberOfBs,NumberOfSs))
    # This is for investigating the -Infs and NAs
    
    #DangerMatrix = array(Zeros,
                         #c(NumberOfIs,NumberOfBs,NumberOfSs))
    ...
    #ParametersAndLogPosteriorDF = as.data.frame(matrix(0,NumberOfParameterSets,2)) #8 ## I added an additional column for S
    ParametersAndLogPosteriorList = list()
    ## There will be a separate list entry for each I.
    ...
    for (INum in 1:NumberOfIs) {  
      ...
      for (SNum in 1:NumberOfSs) {  
        ...
        ##----------------------------------------
        ## This is commented out because I think the number was getting too big to calculate,
        ## so below I calculate it differently:
        #LogLikelihood = -sum( log(1 + exp(-(I + B*x1))) ) - sum( log(1 + exp(I + B*x0)) )
        ...
        #LogLikelihoodMatrix[INum,BNum,SNum] = LogLikelihood
        
        Prior = sqrt( f0(x, d, I, B, S)*f2(x, d, I, B, S) - (f1(x, d, I, B, S))^2 )
        #PriorMatrix[INum,BNum,SNum] = Prior
   ...
   # ResultsDF[4,] = c('Not Applicable', 'Not Applicable', IArray[NumberOfIs], BArray[NumberOfBs],  SArray[NumberOfSs], 'Not Applicable', 'Not Applicable')
    #ResultsDF[5,] = c('Not Applicable', 'Not Applicable', IArray[1], BArray[1], SArray[1], 'Not Applicable', 'Not Applicable')
    
    # ResultsDF[2,] = c(RRNFCITop, RRMaxNFCITop, INFCITop, BNFCITop, SNFCITop, RRMaxToMeanNFCITop,RRMeanToMinNFCITop)
    # ResultsDF[3,] = c(RRNFCIBottom, RRMaxNFCIBottom, INFCIBottom, BNFCIBottom, SNFCIBottom, RRMaxToMeanNFCIBottom, RRMeanToMinNFCIBottom)
    # ResultsDF[4,] = c('Not Applicable', 'Not Applicable', IArray[NumberOfIs], BArray[NumberOfBs],  SArray[NumberOfSs], 'Not Applicable', 'Not Applicable')
    # ResultsDF[5,] = c('Not Applicable', 'Not Applicable', IArray[1], BArray[1], SArray[1], 'Not Applicable', 'Not Applicable')
    # 
    #colnames(ResultsDF) = c('Relative Risk', 'Relative Risk Max', 'Intercept', 'Beta1', 'Beta2', 'Relative Risk Max to Mean', 'Relative Risk Mean to Min')
    #rownames(ResultsDF) = c('MAP', 'Top of 95% CI', 'Bottom of 95% CI', 'Highest Value Tried', 'Lowest Value Tried')
    
    # AllDEGsResultsDF[DEGNum,] = c(CumulativeSumPPDF[1,2], RRNFCIBottom, RRNFCITop,
    #                               CumulativeSumPPDF[1,3], RRMaxNFCIBottom, RRMaxNFCITop,
    #                               RRMaxBestFI, RRMaxNFCIBottomFI, RRMaxNFCITopFI, BinSize,
    #                               CumulativeSumPPDF[1,4], INFCIBottom, INFCITop,
    #                               CumulativeSumPPDF[1,5], BNFCIBottom, BNFCITop,
    #                               CumulativeSumPPDF[1,7], RRMaxToMeanNFCIBottom, RRMaxToMeanNFCITop,
    #                               CumulativeSumPPDF[1,8], RRMeanToMinNFCIBottom, RRMeanToMinNFCITop,
    #                               CumulativeSumPPDF[1,6], SNFCIBottom, SNFCITop)
...
## You look at this after just running FittingOption = 1 (i.e. just the BRGLM fit) and
## it allows you to get a sense of what the parameter value ranges are that should be 
# tried in my custom fit:
...
if (ParallelOption == 0){
  ...
  #                           'OutputStep8With9798', as.character(TaskIDNumber), '.RData', sep = "")
  FileNameForSaving = paste(Folder, '/', StringForOutputFileName, 
