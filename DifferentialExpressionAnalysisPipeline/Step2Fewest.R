...
## Backward selection of technical variables, excluding technical variables 
## that are correlated with the fewest RNAs
## and
...
## This step takes about 24 hours to complete on a laptop (see note at the bottom of this paragraph). 
## First this step corrects the RNA count matrix for sequencing batch and 
## technical variables. Next, it performs differential expression analysis 
## (using DESeq2) on the corrected counts - it simply uses the outcome variable
## in the DESeq2 model and does not include any demographic/clinical covariates
## in the model in this step (and the technical variables have already been used 
## in counts correction, as I just mentioned). The script performs backwards selection 
## to find the set of technical variables that, when used to correct the count matrix, 
## results in the most differentially expressed genes (DEGs) for the outcome variable. 
## We then select that set of technical variables (i.e. the set that yielded the most DEGs).
## (The DEGs identified in this step are not confirmed yet because demographic/clinical covariates have not been included yet - that occurs in Step 3).
...
##==============================================================================
  ...
  ##==============================================================================
    ...
    for (TrialNum in 1:(NumberOfCovariates+1)) { 
      ...
      if (CorrectionOption == 2) {
        ## Loading the corrected counts matrix if you created it previously:
        
        SelectionString = 'Fewest'
      ...
      FileNameCurrentCountsMatrix = 
        paste('CorrectedCountsMatrices/', TypeOfBatch, 'FewestTrial', as.character(TrialNum), '.RData', sep = "")
      ...
      save(CorrectedCountsMatrix, file = FileNameCurrentCountsMatrix)
        ...
        ##==============================================================================
        ## Determining which covariate is least informative:
        ##==============================================================================
        
        if (CorrectionOption < 2) {
          if (length(CovariateNumberToNeverExclude) > 0){
            SmallestNNSP = min(NNSPMatrix[TrialNum, -c((NumberOfCovariates + 1), CovariateNumberToNeverExclude) ])
          } else {
            SmallestNNSP = min(NNSPMatrix[TrialNum, -c(NumberOfCovariates + 1) ])
          }
        
        IndexToRemove = which(NNSPMatrix[TrialNum,] == SmallestNNSP)
  ...
  save(IndexOfBestTrial, DEGList, CovariateResultsDF, NNSPDF, NSQMatrix,
       FinalRecordOfRemovedCovariates, NominalRNAsList,
       file = paste(Folder, '/', StringForOutputFileName, 'OutputStep2Fewest.RData', sep = "") )
  } else {
