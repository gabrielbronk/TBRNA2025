...
for (CovariateNum in 1:NumberOfSignificantCovariates) {
  ...
  ## This might cause some covariates to give NAs for some DEGs in the RNACovariateMatrix, 
  ## but it is what it is because if DESeq2 can't determine whether or not the covariate is correlated with the DEG, 
  ## then there's nothing I can do about it (except maybe running DESeq for more trials,
  ## but I probably don't have to bother with that since I'll just go ahead and run causal inference on 
  ## the covariate/DEG combinations that give NAs). Anyway, the "Checks" at the bottom of this
  ## file will tell me if indeed any RNAs did not converge.
...
## CovariateDEList and NARowsAndColumns are for investigating why some adjp values are NA:
