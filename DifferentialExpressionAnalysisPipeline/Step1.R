...
## (where the outcome is TB progressor vs non progressor, or TB infection vs non-infected, 
...
##==============================================================================
  ...
  CovariatesArray = c('sex', 'child_hiv_birth', 'ethnicity', 'MaternalDepression', 'MaternalPTSD', 
                      ...
                      'household_income_binary', 'Prenatal_smoking_composite')
  ...
  for (d in 1:NumberOfCovsForTable) {
    ...
    ##----------------------------------------------------------------------------
    ## This section obtains the barcodes of individuals in the test group and control group
    ## that have the particular levels of the covariates, so that these barcodes can be used
    ## in the computational experiment of the elastic net where I run the elastic net with
    ## a subsample of individuals have balanced ethnicity (or other covariate) in the TB and 
    ## asymptomatic groups. So this code is here if I ever want to run that kind of experiment again.
    
    BarcodesInTest = Metadata[IndicesOfTest,'Barcode']
    BarcodesInControl = Metadata[IndicesOfControl,'Barcode']
    
    IndicesOf0InTest = which(Metadata[IndicesOfTest,CovariatesArray[d]] == CovariatesArray0[d])
    IndicesOf1InTest = which(Metadata[IndicesOfTest,CovariatesArray[d]] == CovariatesArray1[d])
    BarcodesOf0InTest = BarcodesInTest[IndicesOf0InTest]
    BarcodesOf1InTest = BarcodesInTest[IndicesOf1InTest]
    
    IndicesOf0InControl = which(Metadata[IndicesOfControl,CovariatesArray[d]] == CovariatesArray0[d])
    IndicesOf1InControl = which(Metadata[IndicesOfControl,CovariatesArray[d]] == CovariatesArray1[d])
    BarcodesOf0InControl = BarcodesInControl[IndicesOf0InControl]
    BarcodesOf1InControl = BarcodesInControl[IndicesOf1InControl]
...
##==============================================================================
  ...
  AnovaPValueTableTV = matrix(1000,length(CovariateArray),1)
  ## I shouldn't have used ANOVA. 
  # for (w in 1:length(CovariateArray)) {
  #   MyFormula = as.formula( paste('CaseOrControl ~ ', as.character(CovariateArray[w]), sep='') )
  #   AnovaObject = aov(MyFormula, data = MetadataRTI)
  #   Output = summary(AnovaObject)
  #   AnovaPValueTableTV[w] = Output[[1]][1,5]
  # }
  ...
  for (w in 1:length(CovariateArray)) {
    ...
    AnovaPValueTableTV[w] = TTestObject$p.value
  }
  rownames(AnovaPValueTableTV) = CovariateArray
  colnames(AnovaPValueTableTV) = 'p Value'
