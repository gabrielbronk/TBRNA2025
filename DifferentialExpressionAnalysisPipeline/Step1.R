...
## (where the outcome is TB diseas vs. no disease, TB progressor vs. non progressor, 
## or Mtb infected vs uninfected).
...
##==============================================================================
  ...
  CovariateArray = c('household_income', 'education', 'birth_weight_integral')
...
##==============================================================================
  ...
  CovariateArray = c('birth_weight_integral', 'gestation_delivery','number_in_household')
...
##=========================
## The nominal p-values of the demographic/clinical covariates are thenadjusted for multiple testing using:
  #p.adjust(nominalpvalues, method = 'BH')
##========================  
...
##==============================================================================
  ...
  ## This initializes the TTestPValueTableTV by filling it with 1000's:
  ## TV stands for Technical Variables:
  TTestPValueTableTV = matrix(1000,length(CovariateArray),1)
  for (w in 1:length(CovariateArray)) {
    MyFormula = as.formula( paste(as.character(CovariateArray[w]), ' ~ CaseOrControl', sep='') )
    TTestObject = t.test(MyFormula, data = MetadataBirth, alternative = c("two.sided"))
    TTestPValueTableTV[w] = TTestObject$p.value
  }
  rownames(TTestPValueTableTV) = CovariateArray
  colnames(TTestPValueTableTV) = 'p Value'
  
##=========================
## The nominal p-values of the technical covariates are then adjusted for multiple testing using:
 p.adjust(TTestPValueTableTV[,'p Value'], method = 'BH')
##========================  
...
##==============================================================================
## Seeing how library batch correlates with exonic rate and intergenic rate:
##==============================================================================
  
  MyFormula = as.formula( paste('Library ~ ', 'ExonicRate', sep='') )
  AnovaObject = aov(MyFormula, data = DataForBoxPlot)
  Output = summary(AnovaObject)
  
  MyFormula = as.formula( paste('Library ~ ', 'IntergenicRate', sep='') )
  AnovaObject = aov(MyFormula, data = DataForBoxPlot)
  Output = summary(AnovaObject)
