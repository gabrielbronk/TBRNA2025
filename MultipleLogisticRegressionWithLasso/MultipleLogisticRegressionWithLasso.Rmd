...
if (JobArrayOption == 0) {
  ...
  ## This line is if you are running this script as parallel jobs on a computing cluster, and specifically if you are using the SLURM job scheduler on a computing cluster. (If using a different scheduler, you will need to write this line of code differently). For an example job script for launching this as parallel jobs with SLURM, please see the file ExampleSLURMScriptForParallelJobsOnComputerCluster.sh in this MultipleLogisticRegressionWithLasso folder.
}
